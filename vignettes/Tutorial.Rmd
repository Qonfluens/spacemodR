...
ground_cd <- load_raster_extdata("ground_concentration_cd.tif")
...
# Spacemodel for Risk: a full example: Eco-SSL

## Build a `spacemodel` for Eco-SSL
...
indices <- attr(spcmdl_ecossl_h, "trophic_tbl")
indices <- normalize_weights(indices)
indices$flux <- list(
  function(x) 10^x/32,
  function(x) 10^x/140,
  function(x) 10^x/73,
  function(x) 10^x/0.36,
  function(x) 10^x/0.77
)

spcmdl_ecossl_risk <- transfer(spcmdl_ecossl_h, kernels, indices)
```
