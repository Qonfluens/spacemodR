...
In @veltman2007cadmium the regressions for empirical aboveground plant parts (predominantly leaves) and earthworm concentrations with total soil concentrations is
such as:
...
df_veltman_pars = data.frame(par=c("beta0", "beta1"), value=c(-0.42, 0.59))
ggplot(data=df_pars) +
  theme_minimal() +
  labs(title="Coefficient for logC ~ beta0 + beta1*logCinput (blue=Veltamn's coef)") +
  scale_fill_manual(
    name="parameters",
    values=c("#22aa33", "#aa2233")) +
  geom_density(aes(value, fill=par), alpha=0.5)  +
  geom_vline(data=df_veltman_pars, aes(xintercept=value, group=par), col="blue") +
...
df_veltman_pars = data.frame(par=c("beta0", "beta1"), value=c(1.09, 0.68))
ggplot(data=df_pars) +
  theme_minimal() +
  labs(title="Coefficient for logC ~ beta0 + beta1*logCinput (blue=Veltamn's coef)") +
  scale_fill_manual(
...
In the study of  @veltman2007cadmium, an equation is given by
