...
First, we extract the median (50th percentile) parameter values from our fitted model to use in our equations. Next, we set up the spatial components: we initialize the spacemodel with the soil, herbivore, and insectivore layers, establish the direct trophic links, and set all dispersal kernels to NA (since this is a fixed-point model where individuals do not move). Finally, we define the intake formulas using our extracted parameters and run the transfer() function to propagate the contamination across every pixel of the grid.
