...
- $FIR_{consumer}$: the Food Ingestion Rate in $g food / g bw / day$.
- $k_{met, consumer}$: the coefficient of metabolisation. Default is 1.0 is Terrasys.

The equation of transfer of contaminant from one species to the other is given by:
...
1. We could reduce complexity by assuming a single $k_{upout} = k_{up}/k_{out}$ paramater since parameterisation of 
$k_{up}$ and $k_{out}$ are not identifiable in this equation.
The idea is to re-use the $k_{upout} = k_{met} \times BTF$ from the Terrasys model.
.
2. We also are assuming individuals is old enough to reach a stability of contamination, so as:
$$
...
In our model, ground beetles are plants and earthworm. So we are going to use a transfer model.
...
* $k_{ow}$: octanol /water partition coefficient, with the value of $\log(k_{ow}) = -1.65$ for Cadmium.
