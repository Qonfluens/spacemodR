...
read_sf_safe <- function(file_path) {
  if (file.exists(file_path)) {
    sf_obj <- sf::read_sf(file_path)
    return(sf_obj)
  }
  parent_path <- paste0("../", file_path)
  if (file.exists(parent_path)) {
    sf_obj <- sf::read_sf(parent_path)
    return(sf_obj)
  }
  stop(paste("File not found at", file_path, "and", parent_path))
}
load_safe <- function(file_path) {
  ...
  warning(paste("File not found at", file_path, "or", parent_path))
  return(FALSE)
}
...
# save(fit_direct, file="raw_data/fit_direct_cd.rda")
load_safe("raw_data/fit_direct_cd.rda")
```
...
# save(fit_direct_b100, file="raw_data/fit_direct_b100_cd.rda")
load_safe("raw_data/fit_direct_b100_cd.rda")
```
